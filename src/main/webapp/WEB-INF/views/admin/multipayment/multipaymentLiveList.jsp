    ...
    <script type="text/javascript">
		$(document).ready(function () {
			...
			$('#searchBtn').click(function () {
				...
				var param = jQuery.param({findName: findName, findValue: findValue, status: status, paType: paType, startDt:startDt, endDt:endDt, discount_div:discount_div, mtype:mtype, find_pa_key:find_pa_key});
				location.href = '<%=webRoot%>/admin/multipayment/livelist.do?' + param;
			});
			...
			$('#excelDownBtn').click(function () {
				...
				location.href = '<%=webRoot%>/admin/multipayment/downloadExcelFileLive.do?' + param;
			});
		...
		function function_cancel(sa_id, stb_mac, m_type, pa_key) {
			
			var msg = '해당 고객의 구매건을 취소하시겠습니까?';
			if(confirm(msg)){
				$.ajax({
					url:  "/smartux_adm/admin/multipayment/liveCancel.do",
					type: "POST",
					data: { 
						'sa_id' : sa_id,
						'stb_mac' : stb_mac,
					...
					success: function (rtn) {
						if (rtn.flag=="0000") {
							...
							alert("작업 중 오류가 발생하였습니다\nflag : " + rtn.flag + "\ Message : " + rtn.message+ "\ errorCode : " + rtn.errorCode);
						}
					},
					error: function () {					
                                                                                ...
                                                                                <tr>
                                                                                    ...
                                                                                    <td width="120">
                                                                                        <select class="select" id="status">
                                                                                            ...
                                                                                            <option value="U" <c:if test="${'U' eq vo.status}">selected</c:if>>취소성공(사용자)</option>
                                                                                            <option value="I" <c:if test="${'I' eq vo.status}">selected</c:if>>취소실패(사용자)</option>
                                                                                            <option value="D" <c:if test="${'D' eq vo.status}">selected</c:if>>취소성공(관리자)</option>
                                                                                            <option value="F" <c:if test="${'F' eq vo.status}">selected</c:if>>취소실패(관리자)</option>
                                                                                        </select>
                                                                                    </td>
                                                                                    <td width="120">
                                                                                    ...
                                                                                    <td width="85">
                                                                                        <select class="select" id="findName">
                                                                                            ...
                                                                                            <option value="PROD_ID" <c:if test="${'PROD_ID' eq vo.findName}">selected</c:if>>앨범ID</option>
                                                                                        </select>
                                                                                    </td>
                                                                                    <td>
                                                                    ...
                                                                    <c:forEach var="item" items="${list}" >
                                                                        ...
                                                                        <tr align="center">
                                                                        	...
                                                                        	<td class="table_td_04">
																				<c:if test="${not empty item.pa_status}"> 
																					<c:if test="${item.pa_status ne 'U' and item.pa_status ne 'D'}">
																						<span class="button small blue" onclick="function_cancel('${item.sa_id}','${item.stb_mac}', '${item.m_type}', '${item.pa_key}')">구매취소</span>
																					</c:if>
																				</c:if> 
										                                	</td>
                                                                            <td class="table_td_04">${item.sa_id}</td>
                                                                            <td class="table_td_04">${item.prod_id}</td>
                                                                            <td class="table_td_04"><a class="pointer" style="color: blue" href="./viewLivelist.do?paKey=${item.pa_key}&ptYear=${item.pt_year}&type=${item.pa_type}&ptMonth=${item.pt_month}&pageNum=${vo.pageNum}&findName=${vo.findName}&findValue=${vo.findValue}&status=${vo.status}&paType=${vo.paType}&startDt=${vo.startDt}&endDt=${vo.endDt}&discount_div=${vo.discount_div}&mtype=${vo.mtype}&find_pa_key=${vo.find_pa_key}">${item.prod_name}</a></td>
                                                                            <td class="table_td_04">
                                                                                <c:choose>
                                                                            ...
                                                                            <td class="table_td_04">
                                                                                <c:choose>
			                                                                        ...
			                                                                        <c:when test="${item.pa_status eq 'U'}"><span>취소성공(사용자)</span></c:when>
			                                                                        <c:when test="${item.pa_status eq 'I'}"><span>취소실패(사용자)</span></c:when>
			                                                                        <c:when test="${item.pa_status eq 'D'}"><span>취소성공(관리자)</span></c:when>
			                                                                        <c:when test="${item.pa_status eq 'F'}"><span>취소실패(관리자)</span></c:when>
                                                                                    <c:otherwise>정의되지 않음</c:otherwise>
                                                                                </c:choose>
                                                                            </td>
                                                                ...
                                                                <table border="0" cellpadding="0" cellspacing="0" width="100%">
                                                                    ...
                                                                    <tr>
                                                                        <td align="center">
										                                    ...
										                                    <c:if test="${not empty vo.find_pa_key}">
										                                        <c:set value="${tmpParams}&find_pa_key=${vo.find_pa_key}" var="tmpParams"/>
										                                    </c:if>     
                                                                            <jsp:include page="/WEB-INF/views/include/naviControll.jsp">
                                                                                <jsp:param value="livelist.do" name="actionUrl"/>
