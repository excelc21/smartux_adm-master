...
$(document).ready(function(){
	$("#upbtn").bind("click", function(){
		var idx = $("#contentslist option").index($("#contentslist option:selected"));
		var size = $("#contentslist option").size();
		if(idx == -1){			// 선택한 항목이 없으면
			alert("순서를 바꾸고자 하는 항목을 선택해주세요");
		}else{					// 선택한 항목이 있으면
			fnSortCateogry("contentslist", "U");
		}
	...
	$("#changeorderbtn").click(function(){
		...
		if(confirm("다음의 순서로 정하시겠습니까?")){
			...
			for(var i=0; i < size; i++){
				...
				var selval = $('#contentslist option').eq(i).val();
				optionarray.push(selval);
			}
			
			var smartUXManager = $("#smartUXManager").val();
			var postUrl = "<%=webRoot%>/admin/gpack/contents/updateGpackContentsOrderby.do";
			$.post(  postUrl, 
					 {pack_id : pack_id, category_id : category_id, 
				 	  contents_id : optionarray, smartUXManager : smartUXManager},
					  function(data) {
						                ...
						                <tr>
						                    <td>
						                            ...
						                            <tr>
						                                <td height="25">
						                                    <table border="0" cellpadding="0" cellspacing="0" width="100%">
						                                        <tbody>
						                                        <tr><tr><tr><c:if test="${promotionVO ne null}"><c:if test="${promotionVO.category_nm ne ''}">
					                            ...
					                            <table border="0" cellpadding="0" cellspacing="0" width="300" align="center">
					                                ...
					                                <tr align="center">
					                                    <td width="40%">
					                                    	<select id="contentslist" name="contentslist" size="${fn:length(result)}" style="width:200px; height:200px;">
																<c:forEach var="item" items="${result}" varStatus="status">
																	<option value="${item.contents_id}">${item.contents_nm}</option>
																</c:forEach>
															</select>
														</td>
					                                    <td width="10%"></td>
