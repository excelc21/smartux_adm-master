...
$(document).ready(function(){
	...
	$("#deletebtn").click(function (){
		...
		if(checkeditems.length == 0){
			...
			if(confirm("카테고리를 삭제할 경우 하위 콘텐츠는 자동으로 삭제됩니다.\n선택된 카테고리들을 삭제하시겠습니까?")){
...
function fn_contentsManage(pack_id, category_id, auto_yn){
	...
	if(auto_yn === 'Y'){
		url = "<%=webRoot%>/admin/gpack/contents/getGpackContentsAutoView.do";
		pop_name = "category";
					                            ...
					                            <table border="0" cellspacing="0" cellpadding="0" class="board_list" style="width:100%">
													...
													<c:choose>
														<c:when test="${result==null || fn:length(result) == 0}">
															<tr>
																<td colspan="9" class="table_td_04">검색된 지면이 없습니다</td>
															</tr>
														</c:when>
														<c:otherwise>
															<c:forEach var="item" items="${result}" varStatus="status">
																<tr>
																	...
																	<td class="table_td_04" align="center">
																		<!-- 콘텐츠관리 버튼 -->
																		<span class="button small blue" onclick="javascript:fn_contentsManage('${item.pack_id}', '${item.category_id}', '${item.auto_yn}');">콘텐츠관리</span>
																	</td>
																	<td class="table_td_04" align="center">
																		<!-- 콘텐츠순서관리 버튼 -->
																		<c:if test="${item.auto_yn ne 'Y'}">
