...
$(document).ready(function(){
    ...
    setTerminal();
	/*2010-06-01 수정*/
	if($("#gbn_select").val() == "NEW_MENU"||$("#gbn_select").val() == "REC_MENU"){
		datatypeSelectChnge();
	}
	else{
		datatypeSelectChnge("show");
	}
	...
	$("#gbn_select").change(function(){
		...
		if($(this).val() == "LIVE"){
            ...
            $('#hbanner').val('');
            $('#bannertr').show();
        }else if($(this).val() == "OTT" || $(this).val() == "REC_APP" || $(this).val() == "MIX"){
            $('#link_des').val('');
            $('#link_code').val('');
            $('#linktr').show();
		...
		}else if($(this).val() == "NEW_MENU"||$(this).val() == "REC_MENU"){
			datatypeSelectChnge();
		}
		PageTypeBtnOpenCheck();
	});
...
function init(){
    ...
    if(category_type == "OTT" || category_type == "REC_APP" || category_type == "MIX"){
        $('#linktr').show();
    }
...
function typeBefore(){
	...
	*/
    ...
    }else if(gbnselect == "OTT" || gbnselect == "REC_APP" || gbnselect == "MIX"){
        category_id = $('#link_code').val();
...
function getLinkInfoPop(){
    var link_type = $("#gbn_select").val();
    url = '<%=webRoot%>/admin/mainpanel/getLinkInfoPop.do?linkType='+link_type+'&callbak=getLinkInfoPopCallbak';
    category_window = window.open(url, 'getLinkInfoPop', 'width=670,height=150,left=100,top=50,scrollbars=yes');
}

function getLinkInfoPopCallbak(data){
    console.log(data)
    $("#link_des").val(data.link_des);
...
/* 2020-06-01 수정 : 유재덕 */
function datatypeSelectChnge(chng){
	if("show"==chng){
		$("#noti_message").parent().parent().show();
		$("#message_yn").parent().parent().show();
		$("#message_date").parent().parent().show();
		$("#long_message").parent().parent().show();		
	}
	else{
		$("#noti_message").parent().parent().hide();
		$("#message_yn").parent().parent().hide();
		$("#message_date").parent().parent().hide();
		$("#long_message").parent().parent().hide();
		$("#noti_message").val();
		$("#message_yn").val();
		$("#message_date").val();
		$("#long_message").val();
                    				...
                    				<table border="0" cellpadding="0" cellspacing="0" width="100%" align="center">
						                ...
						                <tr>
						                    <td>
						                        ...
						                        <form id="regfrm" name="regfrm" method="post" action="<%=webRoot%>/admin/mainpanel/getTypeSelect.do" encType="multipart/form-data">
...
<!-- 2019.11.04 : 지면UI타입 추가 - 이태광 End -->															
                                                    ...
                                                    <tr id="linktr" align="center" style="display:none">
                                                        <th>링크정보</th>
                                                        <td></td>
                                                        <td align="left">
                                                            <input type="text"   id="link_des"  name="link_des"  value="${mainresult.category_des}"  maxlength="100" size="20" style="font-size: 12px;" readonly/>
                                                            <input type="hidden" id="link_code"  name="link_code"  value="${mainresult.category_code}"  maxlength="100"  />
                                                            <span class="button small blue" id="searchBtn" onclick="getLinkInfoPop()">선택</span>
                                                        </td>
                                                    </tr>
